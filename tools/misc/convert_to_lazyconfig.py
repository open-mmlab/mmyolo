...
import mmyolo.registry as mmyolo_registry_module
import mmdet.registry as mmdet_registry_module
...
def collect_all_modules():
    mmdet_registries = [value for value in mmdet_registry_module.__dict__.values()
                        if isinstance(value, Registry)]
    mmyolo_registries = [value for value in mmyolo_registry_module.__dict__.values()
                        if isinstance(value, Registry)]
    mmengine_registries = [value for value in mmengine_registry_module.__dict__.values()
                           if isinstance(value, Registry)]
    all_registries = set(mmyolo_registries + mmdet_registries + mmengine_registries)
...
def replace_type_assign(file_content: str, all_modules):
    ...
    for ori_type, real_type in types_dict.items():
        ...
        if not _should_skip(real_type_name, _get_module(real_type_name, all_modules, real_scope)):
            file_content = re.sub(rf"type='?{ori_type}'?", f'type={real_type_name}', file_content)
...
def resolve_special_syntax(filename, file_content):
    ...
    for base_key in base_cfgs:
        ...
        if isinstance(base_cfgs[base_key], dict):
            file_content = file_content.replace(matched_content, f'{base_key}.merge({dict_content})')
        else:
            dict_content = re.sub(r'\_delete\_=True,[\s\n]', '', dict_content)
            file_content = file_content.replace(matched_content, f'{base_key} = {dict_content}')
    # remove _base_ in _base_.xxx.
    # file_content = re.sub(r'(?:\{\{)?(?<!\.)_base_\.([\w\._]*)(?:\}\})?', r'\1', file_content)
    file_content = re.sub(r"(?:\{\{)?(?<!\.)_base_(?:\.([\w\._]*)(?:\}\})?|\['([\w\._]*)'\])", r'\1\2', file_content)
    return file_content
...
def convert_file(filepath, all_modules, target_filepath):
    with open(filepath, 'r') as f:
        ...
        try:
            ...
            file_content, _ = FormatCode(file_content, style_config=yapf_style, verify=True)
        except Exception as e:
...
def convert_files(args):
    ...
    for file, target_file in track(list(zip(all_files, target_files))):
        try:
            ...
            print(
                f'failed to validate file {file} and {target_file} for {e}'
...
def parse():
    ...
    parser.add_argument('--convert', default='all', type=str, help='convert config file')
    parser.add_argument('--validate', default='all', type=str, help='convert config file')
