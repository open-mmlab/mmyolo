...
def qbox2rbox(boxes: np.ndarray) -> np.ndarray:
    ...
    return np.array([x, y, w, h, angle / 180 * np.pi])
    ...
    def get_whs_and_shapes(self):
        ...
        for idx in range(len(self.dataset)):
            ...
            for instance in gt_instances:
                qbbox = np.array(instance['bbox'])
                rbbox = qbox2rbox(qbbox)
                gt_filter_sizes = rbbox[2:4]
