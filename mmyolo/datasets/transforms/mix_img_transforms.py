...
@TRANSFORMS.register_module()
class Mosaic_seg(BaseMixImageTransform):
    """Mosaic augmentation.
    ...
    def mix_img_transform(self, results: dict) -> dict:
        ...
        for i, loc in enumerate(loc_strs):
            ...
            if with_mask and results_patch.get('gt_masks', None) is not None:
                gt_masks_i = results_patch['gt_masks'].resize(
                    results_patch['img_shape'])
                assert isinstance(gt_masks_i, PolygonMasks)
                n = 1000
                # resample_segments()
                resized_masks = []
                c = np.zeros(n * 2)
                for poly_per_obj in gt_masks_i.masks:
                    ...
                    for i in range(n):
                        translated_poly_per_obj.append(
                            [c[2 * i], c[2 * i + 1]])

                    resized_masks.append(translated_poly_per_obj)

                gt_masks_i = PolygonMasks(resized_masks, self.img_scale[0],
                                          self.img_scale[1])

                gt_masks_i.rescale(float([scale_ratio_i, scale_ratio_i]))
        ...
        #         mosaic_masks, *results['img_shape'][:2])
        if with_mask:
