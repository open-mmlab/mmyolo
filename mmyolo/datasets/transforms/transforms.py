...
@TRANSFORMS.register_module()
class YOLOv6RandomAffine(BaseTransform):
    def __init__(self,
                 degrees: float= 10.0,
                 translate=0.1,
                 scale = .1,
    ...
    def transform(self, results: dict) -> dict:
        img = results['img']
        height, width = self.new_shape

        labels = np.concatenate([results['gt_bboxes_labels'][:,None],results['gt_bboxes'].numpy()], axis=1)

        M, s = self.get_transform_matrix(img.shape[:2], (height, width), self.degrees, self.scale, self.shear, self.translate)
        ...
        if n:
            ...
            # filter candidates
            i = self.box_candidates(box1=labels[:, 1:5].T * s, box2=new.T, area_thr=0.1)
            labels = labels[i]
            labels[:, 1:5] = new[i]
    ...
    @cache_randomness
    def get_transform_matrix(self, img_shape, new_shape, degrees, scale, shear, translate):
