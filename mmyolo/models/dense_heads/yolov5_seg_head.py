...
from .yolov5_head import YOLOv5HeadModule, get_prior_xy_info


@MODELS.register_module()
class YOLOv5SegHeadModule(YOLOv5HeadModule):
    ...
    def _init_layers(self):
        ...
        self.conv_pred_seg = nn.Sequential(
            # TODO: 第二个参数要怎么定义
            ConvModule(
                self.in_channels[0],
                256,
    ...
    def forward(self, x: Tuple[Tensor]) -> Tuple[List]:
        ...
        mask_preds = self.conv_pred_seg(x[0])

        return (multi_apply(self.forward_single, x,
                            self.convs_pred), mask_preds)
