...
def select_candidates_in_gts(xy_centers: Tensor, gt_bboxes: Tensor, eps: float = 1e-9) -> Tensor:
    ...
    bbox_deltas = torch.cat([xy_centers - gt_bboxes_lt,
                             gt_bboxes_rb - xy_centers], dim=-1)
    bbox_deltas = bbox_deltas.reshape([batch_size, max_boxes, anchors_number, -1])

    return (bbox_deltas.min(axis=-1)[0] > eps).to(gt_bboxes.dtype)


def select_highest_overlaps(mask_pos: Tensor, overlaps: Tensor, max_boxes: int) -> Tuple[Tensor, Tensor, Tensor]:
...
class BatchATSSAssigner(nn.Module):
    ...
    def forward(self,
        ...
        if max_boxes == 0:
            ...
            return (torch.full([batch_size, total_anchors], self.bg_index).to(device),
                    torch.zeros([batch_size, total_anchors, 4]).to(device),
                    torch.zeros([batch_size, total_anchors, self.num_classes]).to(device),
                    torch.zeros([batch_size, total_anchors]).to(device))

        # calculate overlaps
        overlaps = self.iou2d_calculator(gt_bboxes.reshape([-1, 4]), anchor_bboxes)
        overlaps = overlaps.reshape([batch_size, -1, total_anchors])

        # calculate bbox center distance and anchor points
        distances, anchor_points = bbox_center_distance(gt_bboxes.reshape([-1, 4]), anchor_bboxes)
        distances = distances.reshape([batch_size, -1, total_anchors])

        # filter
        is_in_candidate, candidate_indexes = self.select_topk_candidates(distances, n_level_bboxes, mask_gt)
        overlaps_thr_per_gt, iou_candidates = self.threshold_calculator(is_in_candidate, candidate_indexes, overlaps,
                                                                        total_anchors, batch_size, max_boxes)

        # select candidates iou >= threshold as positive
        is_pos = torch.where(iou_candidates > overlaps_thr_per_gt.repeat([1, 1, total_anchors]),
                             is_in_candidate, torch.zeros_like(is_in_candidate))

        is_in_gts = select_candidates_in_gts(anchor_points, gt_bboxes)
        mask_pos = is_pos * is_in_gts * mask_gt

        target_gt_index, fg_mask, mask_pos = select_highest_overlaps(mask_pos, overlaps, max_boxes)
    ...
    def select_topk_candidates(self,
        ...
        for per_level_distances, per_level_boxes in zip(level_distances, n_level_bboxes):
            ...
            _, per_level_topk_indexes = per_level_distances.topk(selected_k, dim=-1, largest=False)
            candidate_indexes.append(per_level_topk_indexes + start_index)

            per_level_topk_indexes = torch.where(mask_gt,
                                                 per_level_topk_indexes, torch.zeros_like(per_level_topk_indexes))

            is_in_candidate = F.one_hot(per_level_topk_indexes, per_level_boxes).sum(dim=-2)
    ...
    @staticmethod
    def threshold_calculator(is_in_candidate: List,
        ...
        assist_indexes = total_anchors * torch.arange(n_bs_max_boxes, device=candidate_indexes.device)
        ...
        candidate_overlaps = candidate_overlaps.reshape([batch_size, max_boxes, -1])
    ...
    def get_targets(self,
        ...
        target_labels = torch.where(fg_mask > 0,
                                    target_labels, torch.full_like(target_labels, self.bg_index))
