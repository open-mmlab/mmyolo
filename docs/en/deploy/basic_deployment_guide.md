...
More detailed introduction and guides can be found at: https://mmdeploy.readthedocs.io/en/dev-1.x/get_started.html
...
Note: this out-of-box feature is **only available in TensorRT>=8.0**, no need to compile it.
...
To deploy the `YOLOv5` in MMYOLO to `ONNXRuntime`, please use [`detection_onnxruntime_dynamic.py`](../../../configs/deploy/detection_onnxruntime_dynamic.py).
...
To deploy the `YOLOv5` to `TensorRT`, please use [`detection_tensorrt_dynamic-192x192-960x960.py`](../../../configs/deploy/detection_tensorrt-fp16_dynamic-192x192-960x960.py).
