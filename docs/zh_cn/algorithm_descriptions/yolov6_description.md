...
YOLOv6 采用的标签匹配策略采用与 [TOOD](https://arxiv.org/abs/2108.07755)
...
使用 `mmdet.MlvlPointGenerator` 生成anchor points。
...
def distance2bbox(points: Tensor, distance: Tensor, ...) -> Tensor:
    """
        通过距离计算 bbox 的 xyxy
        points (Tensor): 正方形的预测 anchor 的 anchor point [x, y, stride, stride]，Shape (B, N, 4) or (N, 4).
...
1. 对于每一个 `GT`，对所有的 `anchor `基于 **GT类别对应分类分数** 与 **预测框与 GT 的 IoU**
