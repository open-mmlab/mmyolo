...
- `--topk` 只有在`channel_reduction`参数为 None 的情况下，topk 参数才会生效，其会按照激活度排序选择 topk 个通道，然后和图片进行叠加显示，并且此时会通过`--arrangement`参数指定显示的布局，两个数字需要以空格分开，例如：`--topk 5 --arrangement 2 3` 表示以2行3列显示激活度排序最高的5张特征图，`--topk 7 --arrangement 3 3` 表示以3行3列显示激活度排序最高的7张特征图
